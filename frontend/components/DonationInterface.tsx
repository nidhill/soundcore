...
import { Heart, Zap, IndianRupee, Loader2, AlertCircle, CheckCircle2, Mail, X } from 'lucide-react'
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-protest-muted hover:text-protest-text transition-colors"
        >
          <X className="w-4 h-4" />
        </button>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
          <h3 className="font-display text-4xl text-protest-text mb-2 tracking-wider">
            PAYMENT VERIFIED
          </h3>
...
export function DonationInterface() {
  ...
  /* ── Flow state ───────────────────────────────────────────── */
  const [loading,     setLoading]     = useState(false)   // opening checkout
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      /* Step 3 — modal closed: check result then verify server-side */
      if (result?.error) {
        setError('Payment incomplete or verification failed. No funds were deducted.')
        return
      }

      /* Step 4 — strict server-side verification */
      setVerifying(true)
      const verifyRes = await fetch(`${API_URL}/api/verify-payment`, {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
        body:    JSON.stringify({ order_id: orderId, name, email, amount: finalAmount }),
      })
